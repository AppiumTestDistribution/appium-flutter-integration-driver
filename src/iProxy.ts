   ...
   async start() {
      ...
      this.localServer = net.createServer(async (localSocket) => {
         ...
         try {
            ...
            remoteSocket = await utilities.connectPort(
               this.udid,
               this.deviceport,
            );
         } catch (e: any) {
      ...
      this.localServer.once('close', (e: any) => {
         if (e) {
            this.log.info(
               `The connection has been closed with error ${e.message}`,
            );
         } else {
...
class DeviceConnectionsFactory {
   private _connectionsMapping: any;
   constructor() {
      this._connectionsMapping = {};
   }

   _udidAsToken(udid: any) {
      return `${util.hasValue(udid) ? udid : ''}${SPLITTER}`;
   }

   _portAsToken(port: any) {
   ...
   async requestConnection(udid: any, port: any, options: any) {
      if (!udid || !port) {
      ...
      log.debug(
         `Cached connections count: ${_.size(this._connectionsMapping)}`,
      );
               ...
               try {
                  await waitForCondition(
                     async () => {
                        try {
                           if (
                              (await checkPortStatus(port, LOCALHOST)) !==
                              'open'
                           ) {
                              log.info(
